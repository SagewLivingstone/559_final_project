...
export class GrWorld {
    ...
    constructor(params = {}) {
        ...
        // Create an orthographic game camera
        const O_WIDTH = 800 / 30;
        const O_HEIGHT = 600 / 30;
        this.game_camera = new T.OrthographicCamera(
            -O_WIDTH/2, O_WIDTH/2, O_HEIGHT/2, -O_HEIGHT/2, 0.1, 1000
        );
        this.game_camera.name = "Game Camera";
        this.game_camera.position.set(10, 10, 10);
        this.game_camera.lookAt(0, 0, 0);
        ...
        GAME CAMERA
        */
        this.game_controls = new OrbitControls(
            this.game_camera,
            this.renderer.domElement
        );
        this.game_controls.keys = { UP: 87, BOTTOM: 83, LEFT: 65, RIGHT: 68 };
        this.game_controls.mouseButtons = { LEFT: null, MIDDLE: T.MOUSE.ROTATE, RIGHT: null };
        // Force camera at 45*
        const POLAR_ANGLE = Math.PI / 4;
        this.game_controls.minPolarAngle = POLAR_ANGLE;
        this.game_controls.maxPolarAngle = POLAR_ANGLE;
        this.game_controls.maxZoom = 10.0;
        this.game_controls.minZoom = 1.0;
        this.game_controls.zoomSpeed = 3.0;
        // Enable damping
        this.game_controls.enableDamping = true;
        this.game_controls.dampingFactor = 0.5;
        this.game_controls.keyPanSpeed = 20.0;
    ...
    gameControlOn() {
        // TODO: Do we need to handle solo mode? maybe focus on obj but really doesn't matter
        this.game_controls.enabled = true;
        this.active_camera = this.game_camera;
        this.game_controls.update();
        this.game_controls.reset();
    }

    gameControlOff() {
        this.game_controls.enabled = false;
    }
    ...
    remove(grobj)
    {
        if (!(grobj instanceof GrObject)) {
            return;
        }

        for(let i = 0; i < this.objects.length; i++) {
            if (this.objects[0].id == grobj.id)
            {
                this.objects.splice(i, 1);
            }
        }

        // Don't dec objCount because we'll mess up id generation
        // objCount is really an unfortunate name

        // Remove scene objects
        grobj.objects.forEach(o => {
            let found = this.scene.getObjectById(o.id);
            if (found)
                this.scene.remove(found);
        });

        if (this.objNames[grobj.name])
            delete this.objNames[grobj.name];
        
        // FIXME: removed objects still remain in object list in lookat ui!!
    }
    ...
    stepWorld(delta, worldTime) {
        this.objects.forEach(obj => obj.stepWorld(delta, worldTime));
    }
    ...
    */
    ...
    animate(callbacks = {}) {
        ...
        if (!this.runbutton || this.runbutton.checked) {
            ...
            this.worldTime += delta * speed;
            this.stepWorld(delta * speed, this.worldTime);
        ...
        else if ((this.view_mode == "Game Camera") && this.game_controls) {
            this.game_controls.update();
        }
